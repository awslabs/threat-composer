...
export default [
  ...
  // NOTE: the entries below came from .eslintignore, which ESLint 9 flat config no
  // longer supports. They are upstream create-react-app boilerplate, deliberately
  // exempt from both the licence header and the typed rules. Without moving them here
  // the migration silently started linting them and they fail on pre-existing
  // floating promises.
  {
    ignores: ["**/*.js", "**/*.d.ts", "node_modules/", "**/*.generated.ts", "coverage", "build", "dev-dist", "src/service-worker.ts", "src/serviceWorkerRegistration.ts", "src/reportWebVitals.ts"],
  },
  ...
  // shipped with the ESLint 9 migration survived a full `pnpm eslint` run precisely
