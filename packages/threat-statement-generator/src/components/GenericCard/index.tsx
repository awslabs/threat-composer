...
const GenericCard: FC<PropsWithChildren<GenericCardProps>> = ({
  ...
  const actions = useMemo(() => {
    return (<SpaceBetween direction='horizontal' size='s'>
      <Tooltip tooltip='Remove From Workspace'><Button onClick={() => onRemove?.(entityId)} variant='icon' iconName='remove' /></Tooltip>
      <Tooltip tooltip='Edit'><Button onClick={() => onEdit?.(entityId)} variant='icon' iconName='edit' /></Tooltip>
      {moreActions}
    </SpaceBetween>);
