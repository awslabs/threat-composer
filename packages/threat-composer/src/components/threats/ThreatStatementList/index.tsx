...
import {
  GlobalSetupContextApi,
  useGlobalSetupContext,
} from '../../../contexts/GlobalSetupContext/context';
...
import ContentLayoutComponent, {
  ContentLayoutProps,
} from '../../generic/ContentLayout';
...
>
> = ({ composerMode, children, ...props }) => {
  if (composerMode !== 'Full') {
    return <>{children}</>;
  }

  return <ContentLayoutComponent {...props}>{children}</ContentLayoutComponent>;
};
...
const ThreatStatementList: FC<ThreatStatementListProps> = ({
  ...
  const translatedLevelOptionsWithNoValue = useMemo(() => {
    return LevelOptionsWithNoValue.map(status => {
      return {
        label: t(status.label),
        value: status.value,
      };
  ...
  const handleUpdateStatementStatus = useCallback(
    (statement: TemplateThreatStatement, status: string) => {
      saveStatement({
        ...statement,
        status,
      });
  ...
  const filteredStatementList = useMemo(() => {
    ...
    if (selectedStatus && selectedStatus.length > 0) {
      output = output.filter((st) => {
        return st.status
          ? selectedStatus.includes(st.status)
          : selectedStatus.includes(STATUS_NOT_SET);
      });
    }
    ...
    if (selectedSTRIDEs && selectedSTRIDEs.length > 0) {
      output = output.filter((st) => {
        ...
        return (
          stride?.value &&
          (stride.value as string[]).some((tl) => selectedSTRIDEs.includes(tl))
        );
      });
    }
          ...
          >
            <SpaceBetween direction="vertical" size="s">
              <TextFilter
                ...
                filteringPlaceholder={t('Find threat statements')}
                filteringAriaLabel={t('Filter threat statements')}
                onChange={({ detail }) =>
              ...
              <Grid gridDefinition={gridDefinition}>
                <Multiselect
                  ...
                  options={translatedLevelOptionsWithNoValue}
                  placeholder={t('Filtered by priority')}
                ...
                <Multiselect
                  ...
                  placeholder={t('Filtered by STRIDE')}
                  selectedAriaLabel={t('Selected')}
                />
                <TagSelector
                  allTags={allTags}
                ...
                <Multiselect
                  tokenLimit={0}
                  selectedOptions={translatedAllStatus.filter((x) =>
                    selectedStatus.includes(x.value),
                  )}
                  onChange={({ detail }) =>
                    setSelectedStatus(
                      detail.selectedOptions?.map((o) => o.value || '') || [],
                ...
                <Multiselect
                  ...
                  placeholder={t('Filtered by impacted goal')}
                  selectedAriaLabel={t('Selected')}
                />
                {composerMode === 'Full' && (
                  <LinkedEntityFilter
